...
int main() {
    ...
    // 그래프에 초기 노드 추가
    strcpy(graph.nodes[graph.num_nodes].url, "https://www.naver.com/");
    graph.nodes[graph.num_nodes++].num_edges = 0;

    // 추가 페이지 추가 및 연결
    strcpy(graph.nodes[graph.num_nodes].url, "https://www.daum.net/");
    graph.nodes[graph.num_nodes].edges[0] = &graph.nodes[0];
    graph.nodes[graph.num_nodes].num_edges = 1;
    graph.num_nodes++;

    strcpy(graph.nodes[graph.num_nodes].url, "https://map.kakao.com/?referrer=daumtop");
    graph.nodes[graph.num_nodes].edges[0] = &graph.nodes[0];
    graph.nodes[graph.num_nodes].edges[1] = &graph.nodes[1];
    graph.nodes[graph.num_nodes].num_edges = 2;
    graph.num_nodes++;

    strcpy(graph.nodes[graph.num_nodes].url, "https://section.blog.naver.com/BlogHome.naver");
    graph.nodes[graph.num_nodes].edges[0] = &graph.nodes[0];
    graph.nodes[graph.num_nodes].edges[1] = &graph.nodes[1];
    ...
    strcpy(graph.nodes[graph.num_nodes].url, "https://www.tistory.com/");
    graph.nodes[graph.num_nodes].edges[0] = &graph.nodes[0];
    graph.nodes[graph.num_nodes].edges[1] = &graph.nodes[1];
    graph.nodes[graph.num_nodes].edges[2] = &graph.nodes[2];
